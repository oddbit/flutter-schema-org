...
/// The [[ReportageNewsArticle]] type is a subtype of
/// [[NewsArticle]] representing  news articles which are the result
/// of journalistic news reporting conventions  In practice many
/// news publishers produce a wide variety of article types, many of
/// which might be considered a [[NewsArticle]] but not a
...
/// journalism, with articles based on factual information either
/// observed or verified by the author, or reported and verified
/// from knowledgeable sources This often includes perspectives from
/// multiple viewpoints on a particular issue (distinguishing news
/// reports from public relations or propaganda) News reports in the
/// [[ReportageNewsArticle]] sense de-emphasize the opinion of the
/// author, with commentary and value judgements typically expressed
/// elsewhere  A [[ReportageNewsArticle]] which goes deeper into
...
class SchemaReportageNewsArticle implements SchemaSerializable {
  ...
  /// name of the page on which the article is found Please note that
  /// this field is intended for the exact page name (e.g A5, B18).
  String? printPage;
  ...
  /// For an [[Article]], typically a [[NewsArticle]], the backstory
  /// property provides a textual summary giving a brief explanation
  /// of why and how an article was created In a journalistic setting
  /// this could include information about reporting process, methods,
  /// interviews, data sources, etc.  Supported types: [CreativeWork],
  /// [String]
  dynamic backstory;
  ...
  /// 1.) *id-value* URL references - uses *id-value* of an element in
  /// the page being annotated The simplest use of *speakable* has
  /// (potentially relative) URL values, referencing identified
  /// sections of the document concerned  2.) CSS Selectors -
  /// addresses content in the annotated page, e.g via class attribute
  /// Use the [[cssSelector]] property  3.) XPaths - addresses content
  /// via XPaths (assuming an XML view of the content) Use the
  /// [[xpath]] property   For more sophisticated markup of speakable
  /// sections beyond simple ID references, either CSS selectors or
  /// XPath expressions to pick out document section(s) as speakable
  /// For this we define a supporting type, [[SpeakableSpecification]]
  /// which is defined to be a possible value of the *speakable*
  /// property    Supported types: [SpeakableSpecification], [String]
  dynamic speakable;
